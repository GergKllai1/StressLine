...
fun parseArgs(args: Array<String>): RunConfig {
	...
	val method by parser.option(ArgType.String, shortName = "X", fullName = "method", description = "HTTP method")
		.default("GET")
	val headerOpts by parser.option(
		ArgType.String,
		shortName = "H",
		fullName = "header",
	...
	val requests by parser.option(
		ArgType.Int,
		shortName = "n",
		fullName = "requests",
		description = "Stop after N requests",
	)
	val duration by parser.option(
		ArgType.String,
		shortName = "t",
		fullName = "duration",
		description = "Stop after duration e.g. 30s",
	)
	val timeout by parser.option(ArgType.String, fullName = "timeout", description = "Per-request timeout").default("5s")
	val insecure by parser.option(ArgType.Boolean, fullName = "insecure", description = "Skip TLS verification")
		.default(false)
	val noProgress by parser.option(ArgType.Boolean, fullName = "no-progress", description = "Disable live progress")
		.default(false)
	val json by parser.option(ArgType.Boolean, fullName = "json", description = "Emit summary as JSON on stdout")
		.default(false)
	val failIfErrorRate by parser.option(
		ArgType.Double,
		fullName = "fail-if-error-rate",
		description = "Exit 1 if error rate % exceeds this",
	)
	val failIfP95 by parser.option(
		ArgType.String,
		fullName = "fail-if-p95",
		description = "Exit 1 if p95 latency exceeds this duration",
	)
